...
MyD3D::MyD3D(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
	: D3D(screenWidth, screenHeight, vsync, hwnd, fullscreen, screenDepth, screenNear)
{
	...
	D3D11_RASTERIZER_DESC rasterDescWF;
	D3D11_RASTERIZER_DESC rasterDescNoCull;
	...
	rasterDescWF.AntialiasedLineEnable = false;
	rasterDescWF.CullMode = D3D11_CULL_BACK;
	rasterDescWF.DepthBias = 0;
	rasterDescWF.DepthBiasClamp = 0.0f;
	rasterDescWF.DepthClipEnable = true;
	rasterDescWF.FillMode = D3D11_FILL_WIREFRAME;
	rasterDescWF.FrontCounterClockwise = true;
	rasterDescWF.MultisampleEnable = false;
	rasterDescWF.ScissorEnable = false;
	rasterDescWF.SlopeScaledDepthBias = 0.0f;

	m_device->CreateRasterizerState(&rasterDescWF, &m_rasterStateWF);

	//create raster state with back face culling off
	rasterDescNoCull.AntialiasedLineEnable = false;
	rasterDescNoCull.CullMode = D3D11_CULL_NONE;
	rasterDescNoCull.DepthBias = 0;
	rasterDescNoCull.DepthBiasClamp = 0.0f;
	rasterDescNoCull.DepthClipEnable = true;
	rasterDescNoCull.FillMode = D3D11_FILL_SOLID;
	rasterDescNoCull.FrontCounterClockwise = true;
	rasterDescNoCull.MultisampleEnable = false;
	rasterDescNoCull.ScissorEnable = false;
	rasterDescNoCull.SlopeScaledDepthBias = 0.0f;

	m_device->CreateRasterizerState(&rasterDescNoCull, &m_rasterStateNoCull);
...
MyD3D::~MyD3D()
{
	...
	if (m_rasterStateNoCull)
	{
		m_rasterStateNoCull->Release();
		m_rasterStateNoCull = 0;
	}
...
void MyD3D::TurnOnCulling(bool WF)
{
...
void MyD3D::TurnOffCulling()
{
	// Set the no back face culling rasterizer state.
	m_deviceContext->RSSetState(m_rasterStateNoCull);
