...
void App1::init(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight, Input *in)
{
	...
	m_Camera->SetRotation(11.5f, 48.0f, 0.0f);
	...
	m_Sunlight->GenerateProjectionMatrix(1.0f, 1000.0f);
	...
	while (i != localVec.end())
	{
		...
		if (m_CameraFrustum->CheckRectangle((*i)->boundingBox.maxX, (*i)->boundingBox.maxY, (*i)->boundingBox.maxZ, (*i)->boundingBox.minX, (*i)->boundingBox.minY, (*i)->boundingBox.minZ) && (*i)->toRender)
		{
			...
			m_TessellationShader->SetShaderParameters(m_Direct3D->GetDeviceContext(), worldMatrix, viewMatrix, projectionMatrix, // Matrices
														 m_Terrain->Heightmap()->GetTexture(), m_Terrain->GetTerrainRockTexture()->GetTexture(), m_Terrain->GetTerrainSnowTexture()->GetTexture(), m_Terrain->GetNormalMap()->GetTexture(), m_DepthMap->GetShaderResourceView(), // Textures and maps
...
void App1::OcclusionTest()
{
	...
	// Turn off screen rendering and depth buffer writing
	m_Direct3D->TurnZBufferOff();	
	...
	{
			// Begin query
			m_Direct3D->GetDeviceContext()->Begin(occlusionQuery);
			...
			// Get number of vertices rendered for each object
			UINT64 occlusionData = 0;
			m_Direct3D->GetDeviceContext()->GetData(occlusionQuery, &occlusionData, sizeof(UINT64), 0);

			if (occlusionData == 0)
			{
				(*i)->toRender = false;
			}
			else if (occlusionData > 0)
			{
				(*i)->toRender = true;
			}
	}	

	// Turn on screen rendering and depth buffer writing
	m_Direct3D->TurnZBufferOn();
}
...
void App1::RenderShadowMap()
{
	...
	while (i != localVec.end())
	{
		...
		m_DepthShader->SetShaderParameters(m_Direct3D->GetDeviceContext(), worldMatrix, lightViewMatrix, lightProjectionMatrix, 
										   m_Terrain->Heightmap()->GetTexture(), m_Camera, m_MultifractalData, tessDistNear, tessDistFar);
		//// Render object (combination of mesh geometry and shader process
...
void App1::gui()
{
	...
	ImGui::SliderFloat("Near", &tessDistNear, 20.0f, tessDistFar);
	ImGui::SliderFloat("Far", &tessDistFar, tessDistNear, 300.0f);
